 ...
 *   $radius = new RadiusClient($ip_radius_server = 'radius_server_ip_address', $shared_secret = 'radius_shared_secret'[, $radius_suffix = 'optional_radius_suffix'[, $udp_timeout = udp_timeout_in_seconds[, $authentication_port = 1812]]]);
 ...
 *     <?php
 *         require_once('radius.class.php');
 *         $radius = new RadiusClient('127.0.0.1', 'secret');
...
class RadiusClient
{
    ...
    function GetParsedReceivedAttributes()
    {
        ...
        if (isset($this->_attributes_received))
        {
            foreach($this->_attributes_received as $one_received_attribute)
            {
                ...
                if (26 == $one_received_attribute[0])
                {
                    ...
                    {
                        $parsed_attributes[] = array(
							'Attribute' => $attributes_info[0], 'Value' => $one_received_attribute[1],
							'Vendor-Id' => $vendor_one[0],
							'Vendor-Type' => $vendor_one[1],
							'Attribute-Specific' => $vendor_one[2],
						);
                    }
                }
                else
                {
                    $parsed_attributes[] = array('Attribute' => $attributes_info[0], 'Value' => $one_received_attribute[1]);
                }
            }
        }
